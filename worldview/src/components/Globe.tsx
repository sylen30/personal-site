...
export function GlobeView() {
  ...
  useEffect(() => {
    ...
    // Replace the default imagery with Cesium World Imagery (or Bing fallback).
    if (ION_TOKEN) {
      createWorldImageryAsync()
        .then((provider) => {
      ...
      // Optional Google Photorealistic 3D Tiles via Cesium Ion (asset 2275207)
      // Toggle on if user provides a Google API key — uses the public tile set.
      if (GOOGLE_3D_KEY) {
      ...
      // No Ion token — use a free natural earth imagery provider as fallback.
      void IonImageryProvider.fromAssetId(3845)
        .then((provider) => {
          viewer.imageryLayers.removeAll();
          viewer.imageryLayers.addImageryProvider(provider);
        })
